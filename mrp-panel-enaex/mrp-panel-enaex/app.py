...
APP_VERSION = "2026-07-30d · criticidad B (diagnóstico)"
...
def pagina_mrp_e002():
    ...
    with t1:
        ...
        # Diagnóstico en vivo de la criticidad. Sirve para confirmar QUÉ código
        # está corriendo: con la versión nueva, "Sin criticidad" debe ser 0 y
        # los materiales sin clase aparecen como "Media" (clase B). Si aquí ves
        # 0 en "Sin criticidad" pero el gráfico de abajo muestra una barra de
        # "Sin criticidad", significa que la vista quedó cacheada: reinicia la app.
        if "Criticidad texto" in datos.columns:
            vc = datos["Criticidad texto"].value_counts()
            _alta = int(vc.get("Alta", 0)); _media = int(vc.get("Media", 0))
            _baja = int(vc.get("Baja", 0)); _sin = int(vc.get("Sin criticidad", 0))
            st.caption(
                f"🔎 Criticidad (semana actual): **Alta** {_alta} · "
                f"**Media (clase B)** {_media} · **Baja** {_baja} · "
                f"Sin criticidad: {_sin}  ·  versión {APP_VERSION}"
            )
